...
const dfFontSize = 30
const uploadurl = app.globalData.uploadurl
Component({
  /**
   * 组件的属性列表
   */
  ...
  methods: {
        ...
        success: function(res) {
          ...
          for (var item of res.tempFilePaths) {
            ...
            that._uploadImage(item,res=>{
              wx.hideLoading()
              //上传图片的回调
              if(res.status = 'y'){//表示有数据返回，这里的数据是指服务器返回的图片网络地址，进行数据赋值
                that._dealDataArray(item,res,1)
                console.log('上传图片的回调成功:', res)
              }else{
                //上传失败，需要更新数据
                that._dealDataArray(item,'',-1)
                console.log('上传图片的回调失败:', res)
              }
            })
            /**
    ...
    imgUploadAginEvent(e){
      ...
      this._uploadImage(imgPath,res=>{
        ...
        if(retinfo.status = 'y'){
          //上传成功，改变数据源
          imgItem['imgUrl'] = retinfo.result.img_url
          ...
          this._checkEditData(res => { })
          console.log('上传成功，改变数据源 imgUploadAginEvent completed:', that.data.dataAry)
        }else{  
          // 上传失败，提示用户
          wx.showToast({
            title: '图片上传失败',
    ...
    deleteImg(e) {
      var that = this
      var index = e.currentTarget.dataset.index;
      var imgItem = that.data.dataAry[index]
      var imgUrl = imgItem.imgUrl
      console.log('deleteImg index:', index, ' dataAry:', that.data.dataAry, ' image url:', that.data.dataAry[index]['imgUrl'])
      var dic1 = that.data.dataAry[index - 1]
      var dic2 = that.data.dataAry[index + 1]
      //图片前后都有数据存在
      if (dic1 != undefined && dic2 != undefined) {
        ...
        if (dic1.type == 'string' && dic2.type == 'string') {
          ...
          that.data.dataAry.splice(index - 1, 3, dic3)
        } else { //删除图片前后是图片和文字时
          that.data.dataAry.splice(index, 1)
        }
      } else { //图片前面没有数据时
        that.data.dataAry.splice(index, 1)
      }
      ...
      that._checkEditData(res => { })
      // 删除方法传到外部，不管外部怎么操作，本界面删除的图片已经从数据源中删除。（外部可做删除网络图片的操作）
      that.triggerEvent('graphicDeleteImg', imgUrl)
    },
    // 对文本编辑的内容进行提交
    submit(e) {
    ...
    */
    _dealContentText(content) {
      ...
      if (content.length > 0) {
        ...
        console.log(' GraphiceEditPage _dealContentText dataAry:', this.data.dataAry)
        // 判断当前数据是否可提交
        this._checkEditData(res => { })
      }
    },
    ...
    _uploadImage(filePath, callBack) {
      ...
      wx.uploadFile({
        ...
        formData: {
          ...
          city: encodeURI('杭州'),
          prov: encodeURI('浙江'),
          title: encodeURI('card image'), // 名称
        }, // HTTP 请求中其他额外的 form data
        success(res){
          //上传成功回调
          var retinfo = JSON.parse(res.data.trim());
          if (retinfo.status == 'y'){
            let imgUrl = retinfo.result.img_url
            console.log('image url:', imgUrl,' server url:',that.properties.uploadImageUrl)
            callBack(retinfo)
          }else{
            console.log('上传成功回调 retinfo:', retinfo, ' server url:', that.properties.uploadImageUrl)
            callBack(retinfo)
          }
        },
        fail(error){
          // 上传失败回调
          console.log('上传失败回调 server url:', that.properties.uploadImageUrl)
          callBack('')
        }
      })
    },
